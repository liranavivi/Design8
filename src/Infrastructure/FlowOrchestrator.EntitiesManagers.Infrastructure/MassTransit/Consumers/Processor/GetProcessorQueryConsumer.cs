...
public class GetProcessorQueryConsumer : IConsumer<GetProcessorQuery>
{
    ...
    public GetProcessorQueryConsumer(IProcessorEntityRepository repository, ILogger<GetProcessorQueryConsumer> logger)
    {
    ...
    {
        using var activity = Activity.Current?.Source.StartActivity("GetProcessorQuery");
        ...
        try
        {
            if (context.Message.Id.HasValue)
            {
                ...
                if (entity != null)
                    ...
                    await context.RespondAsync(new { Error = "Processor not found", Type = "NotFound" });
            }
            ...
            _logger.LogInformation("Successfully processed GetProcessorQuery");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing GetProcessorQuery");
