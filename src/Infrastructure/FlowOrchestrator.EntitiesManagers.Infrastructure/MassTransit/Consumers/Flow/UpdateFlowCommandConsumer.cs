...
public class UpdateFlowCommandConsumer : IConsumer<UpdateFlowCommand>
{
    private readonly IFlowEntityRepository _repository;
    private readonly IPublishEndpoint _publishEndpoint;
    private readonly ILogger<UpdateFlowCommandConsumer> _logger;

    public UpdateFlowCommandConsumer(
        IFlowEntityRepository repository,
        IPublishEndpoint publishEndpoint,
        ILogger<UpdateFlowCommandConsumer> logger)
    {
    ...
    public async Task Consume(ConsumeContext<UpdateFlowCommand> context)
    {
        _logger.LogInformation("Processing UpdateFlowCommand for ID {Id}", context.Message.Id);
        ...
        try
        {
            ...
            if (existing == null)
            {
                _logger.LogWarning("FlowEntity with ID {Id} not found for update", context.Message.Id);
            ...
            await _publishEndpoint.Publish(new FlowUpdatedEvent
            {
                Id = updated.Id,
            ...
            _logger.LogInformation("Successfully processed UpdateFlowCommand for ID {Id}", context.Message.Id);
        }
        catch (DuplicateKeyException ex)
        {
            _logger.LogWarning("Duplicate key error in UpdateFlowCommand: {Error}", ex.Message);
            await context.RespondAsync(new { Error = ex.Message, Success = false });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing UpdateFlowCommand for ID {Id}", context.Message.Id);
            throw;
        }
    }
}
