...
# Final AddressesController API Tests
...
echo "=========================================="
echo "FINAL ADDRESSES CONTROLLER API TESTS"
echo "=========================================="
echo ""
...
echo "Creating schema with JSON:"
echo "$SCHEMA_JSON"
echo ""
...
echo "Schema creation response code: $SCHEMA_RESPONSE"
echo "Schema creation response body:"
cat /tmp/schema_response.json
echo ""

if [ "$SCHEMA_RESPONSE" = "201" ]; then
    # Extract schema ID using simple text processing
    ...
    echo "Using a known existing schema ID for tests..."
    # Use the existing address's schema ID from the GET all response
...
# Use the created schema ID if available, otherwise use a known working one
if [ "$SCHEMA_ID" != "00000000-0000-0000-0000-000000000000" ] && [ -n "$SCHEMA_ID" ]; then
    WORKING_SCHEMA_ID="$SCHEMA_ID"
else
    # Get an existing schema ID from the system
    echo "Getting existing schema for testing..."
    curl -s "$SCHEMAS_URL" -o /tmp/existing_schemas.json
    WORKING_SCHEMA_ID=$(cat /tmp/existing_schemas.json | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
    echo "Using existing schema ID: $WORKING_SCHEMA_ID"
fi
...
VALID_JSON='{
  ...
  "address": "test-address-unique-'$TIMESTAMP'",
  "schemaId": "'$WORKING_SCHEMA_ID'"
}'
echo "Creating address with JSON:"
echo "$VALID_JSON"
response=$(curl -s -w "%{http_code}" -X POST \
  -H "Content-Type: application/json" \
  -d "$VALID_JSON" \
...
if [ "$response" = "201" ]; then
    CREATED_ADDRESS_ID=$(cat /tmp/response.json | grep -o '"id":"[^"]*"' | cut -d'"' -f4)
    CREATED_ADDRESS_VALUE=$(cat /tmp/response.json | grep -o '"address":"[^"]*"' | cut -d'"' -f4)
...
VALID_UPDATE_JSON='{
  ...
  "address": "updated-test-address-unique-'$UPDATE_TIMESTAMP'",
  "schemaId": "'$WORKING_SCHEMA_ID'"
}'
response=$(curl -s -w "%{http_code}" -X PUT \
  -H "Content-Type: application/json" \
...
if [ "$response" = "200" ]; then
    UPDATED_ADDRESS_VALUE=$(cat /tmp/response.json | grep -o '"address":"[^"]*"' | cut -d'"' -f4)
...
if [ -n "$UPDATED_ADDRESS_VALUE" ]; then
...
# Cleanup - Delete the test schema if we created one
if [ "$SCHEMA_ID" != "00000000-0000-0000-0000-000000000000" ] && [ -n "$SCHEMA_ID" ]; then
    echo "=========================================="
